...
def add_tag():
    if layout_notes.selectedItems():
        ...
        print("Заметки для добавления тега не выбраны!")
...
def del_tag():
    if list_tags.selectedItems():
        ...
        list_tags.addItems(notes[key]["теги"])
        with open("notes_data.json", "w") as file:
            json.dunp(notes, file, sort_keys=True, ensure_ascii=False)
        print(notes)
    else:
    ...
    if button_tag_search.text() == "Искать заметки по тегу" and tag:
        ...
        for note in notes:
            if tag in notes[note]["теги"]:
                notes_fildered[note]=notes[note]
        button_tag_search.setText("Сбросить поиск")
        list_notes.clear()
        list_tags.clear()
        list_notes.addItems(notes_fildered)
        print(button_tag_search.text())
